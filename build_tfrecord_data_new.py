...
def read_sentences(filename):
    ...
    with codecs.open(filename, encoding='utf8') as f:
        for line in f:
            ...
            labels.append([0, 1] if label.startswith('R_PLOC') else [1, 0])
...
def sentence_matrix(sentences,
    ...
    for sent in sentences:
        ...
        for (word, pos, ent_type, to_e1, to_e2, dep_label) in sent:
            ...
            if mask_p1p2 and (ent_type in ['P1', 'P2','P1P2']):
                word = '_ARG_ENTITY_'
...
def load_sentence_matrix_v1_new(filename):
    sentences, head_sents, dep_sents, sent_entity, labels = read_sentences(filename)
    sent_mx = sentence_matrix(sentences)
    head_mx = sentence_matrix(head_sents)
    dep_mx = sentence_matrix(dep_sents, 10)
...
def load_context_matrix_v1_new(filename):
    sentences, head_sents, dep_sents, sent_entity, labels = read_sentences(filename)
    left_mx, middle_mx, right_mx = context_matrix(sentences)
    dep_mx = sentence_matrix(dep_sents, 10)
    entity_mx = sentence_matrix(sent_entity)
...
if __name__ == '__main__':
    ...
    for folder_name in ['baseline','CP','CP_TW','CP_HP','filtered']:
        build_sent_dataset('data/ds_new/'+folder_name+'_train_dev.txt','data/pretrain/'+folder_name+'/'+folder_name+'_train_dev.tfrecords')
        build_sent_dataset('data/ds_new/'+folder_name+'_train.txt','data/pretrain/'+folder_name+'/'+folder_name+'_train.tfrecords')
        split_tfrecord_file('data/pretrain/'+folder_name+'/'+folder_name+'_train.tfrecords', 'data/pretrain/'+folder_name, 128)
    '''


    #build data for PCNN
    for folder_name in ['baseline_top','CP_top','CP_HP_top','CP_TW_top','filtered_top']: #'baseline','CP','CP_HP','HP'
    ...
    '''
    #build html dev data for PCNN
    build_cont_dataset('data/html_dev.txt','data/html_dev.tfrecords')
