...
def show_train_figure(folder_name,train_f_score,interval):
    ...
    plt.title('Overall recall on test set of '+folder_name, fontsize=20)
    #plt.ylim(0.73,0.75)
    plt.xlabel('Epoch Time', fontsize=16)
    plt.ylabel('F Score', fontsize=16)
    plt.show()
...
if __name__ == '__main__':
    ...
    hypep='_mc_2'
    for folder_name in ['filtered']: #'baseline','CP','CP_TW','CP_HP','filtered','no_pretrain'
        print(folder_name)
        #folder_name_test="CP"
        pickle_file_path="./model_cnn_new/"+folder_name+"/"
        print('Precision')
        train_f_score=load_pickle(pickle_file_path,'test_precision_doc_new'+hypep+'.pickle')
        #print(train_f_score)
        #print(len(train_f_score))
        show_train_figure(folder_name,train_f_score,interval)
        print('Recall')
        train_f_score=load_pickle(pickle_file_path,'test_recall_doc_new'+hypep+'.pickle')

        show_train_figure(folder_name,train_f_score,interval)
        print('F score')
        train_f_score=load_pickle(pickle_file_path,'test_fscore_doc_new'+hypep+'.pickle')
